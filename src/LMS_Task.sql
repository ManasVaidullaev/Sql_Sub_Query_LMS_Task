...
alter table rent_infos rename to rent_info;
...
select house_type,first_name,last_name,rating from houses inner join owners o on o.id = houses.owner_id order by rating desc ;
select house_type,first_name,last_name,rating from houses inner join owners o on o.id = houses.owner_id order by rating  ;
-- 7. Уйлордун арасынан квартиралардын (apartment) санын жана алардын баасынын суммасын чыгар.
select house_type,count(*),sum(price) from houses where house_type='Apartment' group by house_type;
...
select city,region,street,house_id from agencies inner join addresses a on a.id = agencies.address_id inner join rent_info ri on agencies.id = ri.agency_id where name='My House';
-- 9. Уйлордун арасынан мебели бар уйлорду, уйдун ээсин жана уйдун адрессин чыгар.
select first_name,last_name, city,region,street, furniture from houses inner join owners o on houses.owner_id = o.id inner join addresses a on a.id = houses.address_id where furniture=true;
...
select gender,first_name  from customers group by  gender, first_name;
select first_name,gender from owners group by first_name, gender;
-- 21.Эн коп моонотко  ижарага алынган уйду чыгарыныз.

-- 22.Эн кымбат уйду жана анын ээсин чыгарыныз.
select first_name,last_name,price from houses inner join owners o on houses.owner_id = o.id order by price desc limit 1;
-- 23.Бир региондо жайгашкан баардык агентстволорду чыгарыныз
select *from agencies inner join addresses a on a.id = agencies.address_id where region='Alamedin';
